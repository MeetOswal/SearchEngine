...
void compress(std::string directory)
{
    ...
    string outfileName = std::to_string(shard) + "/compressed_inverted_index.bin";
    string outfileLexiconName = std::to_string(shard) + "/lexicon_index.txt";
    ofstream outFile(outfileName, std::ios::binary);
    ofstream outFileLexicon(outfileLexiconName, std::ios::app);
    ...
    string filename = directory + "0.txt";
    remove(filename.c_str());
...
void structureChange(string directory)
{
    ...
    // Open two output files: one for compressed data and one for the lexicon
    string outfileName = std::to_string(shard) + "/inverted_index.txt";
    string outfileLexiconName = std::to_string(shard) + "/word_list.txt";
...
int main()
{
    ...
    for (int i = 0; i < SHARDS; i++)
    {
        ...
        shard = i;
        // Create a folder for the current shard
        std::string folderPath = std::to_string(shard);
        ...
        // Construct the file path for the current shard's input file
        string fileName = "trecFiles/cluster_" + std::to_string(shard) + "_output.trec";
        // Initialize a map to store document IDs
        unordered_map<int, string> DocID;
        // create subindex
        // Read the input file and get the total number of files
        int totalFiles = readFile(fileName, &DocID);

        // int totalFiles = 30;
         // Set the batch size for merging subindexes
        int batchSize = 0;
        if (totalFiles >= 100)
        {
        ...
        // Vector to store intermediate filenames
        vector<string> filenames;
        int intermediateTotal = 0;

        // 3214
        // Split the total files into batches and merge them into intermediate indexes
        for (int batchStart = 1; batchStart <= totalFiles; batchStart += batchSize)
        {
            vector<string> fileBatch;
            // Create a batch of file paths
            for (int i = batchStart; i < batchStart + batchSize && i <= totalFiles; i++)
            {
                fileBatch.push_back("subindex/" + std::to_string(i) + ".txt");
            }
            // Construct the intermediate file path
        ...
        // 33
        // Perform merge sort loop until a single final list is obtained
        ...
        while (true)
        {
            if (value)
            {
                // Merge sort loop with "intermediate/" as input and "subindex/" as output
            ...
            {
                // Merge sort loop with "subindex/" as input and "intermediate/" as output
